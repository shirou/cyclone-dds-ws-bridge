...
- [ ] `protocol.go` -- message header and payload encode/decode
  - [ ] Header struct, MsgType constants
  - [ ] Version check matching bridge behavior
  - [ ] Encode/decode helpers for each message type
  - [ ] String encoding: 4-byte LE uint32 length + UTF-8
  - [ ] QoS and KeyDescriptors binary representation
  - [ ] KeyDescriptors builder helper: given field offsets and types, construct binary representation
- [ ] `client.go` -- WebSocket client
  - [ ] `Client` struct wrapping a WebSocket connection
  - [ ] `Dial(ctx, addr) -> (*Client, error)`
  - [ ] `Subscribe(topic, typeName, qos, keyDesc) -> error`
  - [ ] `Unsubscribe(topic, typeName) -> error`
  - [ ] `Write(topic, typeName, qos, keyDesc, data) -> error`
  - [ ] `WriteWithID(writerID, data) -> error`
  - [ ] `CreateWriter(topic, typeName, qos, keyDesc) -> (writerID, error)`
  - [ ] `DeleteWriter(writerID) -> error`
  - [ ] `Dispose(topic/writerID, keyData) -> error`
  - [ ] `Recv() -> (Message, error)`
  - [ ] `Ping() -> error` / `Close() -> error`
- [ ] `client_test.go` -- protocol encode/decode round-trip tests
- [ ] `go.mod` -- module `github.com/shirou/cyclone-dds-ws-bridge/ddswsclient`
- [ ] Thread-safe: multiple goroutines can call Write/Subscribe concurrently
- [ ] request_id tracking: auto-assign unique IDs (never 0), correlate OK/ERROR responses
- [ ] Protocol conformance: verify against shared `.bin` test vectors from Phase 1-5

### 5-2. Example

- [ ] `example_test.go` -- usage example showing intended workflow
...
- [ ] `tests/interop/python/generate_fixtures.py` -- serialize test data with pycdr2
- [ ] `tests/interop/python/verify_go_fixtures.py` -- deserialize Go-generated .bin with pycdr2
- [ ] `tests/interop/python/requirements.txt`
- [ ] `tests/interop/go/generate.go` -- go:generate directive to run go-dds-idlgen on IDL
- [ ] `tests/interop/go/interop_test.go` -- decode Python fixtures, encode Go fixtures, byte-exact comparison
- [ ] `tests/interop/run_interop_tests.sh` -- orchestrates the full pipeline
- [ ] Test data: `sensor::SensorData`
- [ ] Test data: `interop::PrimitiveTypes` (boundary values, zero values)
- [ ] Test data: `interop::KeyedMessage`
- [ ] Test data: `interop::WithSequence` (non-empty and empty)
- [ ] Test data: `interop::WithEnum`
...
Phase 5   (Go client lib)   --conformance + unit tests pass-->
Phase 6-1 (interop L1)      --CDR compatibility verified-->
Phase 6-2 (interop L2)      --docker-compose E2E pass-->
Phase 7   (integration + CI + polish)
```

Phases 0-4 are complete (86 unit tests pass). Next: Phase 5 (Go client library).
Phase 5 is DDS-independent and only depends on the protocol specification.
E2E testing of Phase 3/4 will be done in Phase 7 (integration tests).
